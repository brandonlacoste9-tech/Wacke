...
interface AIRoastBattleProps {
  streamerName: string;
}

export default function AIRoastBattle({ streamerName }: AIRoastBattleProps) {
  ...
  const startBattle = async () => {
    setLoading(true);
    try {
      // Call AI twice for two roasts, then decide winner
      ...
      const judgeRes = await fetch("/api/AI", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: judgePrompt, maxTokens: 60 }),
      });
      const judgeData = await judgeRes.json();
      const judge = judgeData.content || "1 wins because AI said so.";
      ...
      // Speak the winner with real AI voice
      speakWithCloudAIVoice(`Winner: ${finalBattle.winner}`, language);
    } catch (e) {
      const fallback = {
        roast1: "AI a planté le premier roast.",
        roast2: "Le deuxième était pire.",
        winner: "Personne ne gagne, c'est un tie wacké.",
      };
      setBattle(fallback);
      speakWithCloudAIVoice(fallback.winner, language);
    }
    setLoading(false);
  };

  const speakRoast = (text: string) => {
    speakWithCloudAIVoice(text, language);
  };
  ...
  return (
    <div className="glass p-4 rounded-xl border border-white/[0.07]">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-bold text-sm font-display tracking-wide">{language === "fr" ? "AI ROAST BATTLE" : "AI ROAST BATTLE"}</h3>
      </div>
      <button
        onClick={startBattle}
        disabled={loading}
      ...
      >
        <Flame /> {loading ? (language === "fr" ? "AI JUGE..." : "AI IS JUDGING...") : (language === "fr" ? "DÉMARRER LE ROAST BATTLE" : "START ROAST BATTLE")}
      </button>
      ...
      {battle && (
        <div className="mt-3 space-y-2 text-xs">
          ...
          <div className="text-[10px] text-center text-gray-400">
            {language === "fr" ? "Jugé par AI xAI • Maximum wacké" : "Judged by AI xAI • Maximum wacké"}
          </div>
        </div>
      )}
    </div>
