...
export default function HomePageClient({ initialStreams, initialStats }: HomePageClientProps) {
  const { t } = useLanguage();
  ...
  return (
    <main className="min-h-screen bg-wacke-dark">
      ...
      <QuebecLiveRow streams={initialStreams.all} />

      <section className="px-6 lg:px-8 pb-12 max-w-7xl mx-auto">
        <CombinedStreamGrid limit={20} initialStreams={initialStreams} />
      </section>
          ...
          {categories.map((cat) => (
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center p-4 z-20">
                <div className="text-3xl mb-2 group-hover:scale-110 transition-transform duration-300 drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)]">
                  {cat.icon}
                </div>
                <p className="text-sm font-black text-white uppercase tracking-tight drop-shadow-[0_1px_4px_rgba(0,0,0,0.8)]">
                  {cat.name}
                </p>
              </div>
            </Link>
